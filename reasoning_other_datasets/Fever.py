...
# =================================================================
# FEVER evaluation helper functions
# =================================================================
...
def f1_score(prediction, ground_truth):
    """Compute the token-level F1 score used in ReAct-style evaluation."""
...
class ReActSpeculativeEngine:
    def __init__(
        ...
        self.http_headers = {
            'User-Agent': 'FEVER-SpeculativeEngine/1.0 requests-library'
        }
        self.request_timeout = (1.5, 6.0)

        print(">>> Loading large model (70B AWQ)...")
        ...
        # =================================================================
        # State machine initialization with EOT handling
        # =================================================================
        self.nl_id = self._encode_text("\n")[-1]
        self.eos_id = self.tokenizer.eos_token_id
        # Handle the Llama-3.1 specific stop token
        ...
        self.log_csv = "final_integrated_results_fever1.csv"
        ...
        # Keep the online Wikipedia retrieval mode instead of loading a local corpus
    ...
    def _render_trace_summary(self, max_chars):
        ...
        header = (
            ...
            "[S-ACC] = drafted by the small model and accepted by the large model | "
            "[B-DIR] = generated directly by the large model | "
            "[B-RES] = small-model draft rejected and resampled by the large model\n"
            + "-" * 68 + "\n"
        )
        parts = []
    ...
    def _load_wiki_corpus(self):
        print(">>> Online Wikipedia API retrieval is enabled.")
        if self.enable_ste:
            print(">>> STE is enabled: tool results will be prefetched asynchronously when the small model drafts a complete Action.")
        if self.enable_hidden_probe:
            print(">>> Hidden Probe is enabled: the draft stage probes Action queries early and prefetches them in advance.")
        return None
    ...
    def _extract_search_query(self, text):
        # Accept quoted queries such as SearchWikipedia["Query"] as well.
    ...
    def _parse_action_query(self, action_buffer):
        ...
        # Fallback for outputs such as `Action: Query` without brackets.
    ...
    def _build_observation_text(self, query: str, docs: List[Dict[str, Any]]) -> str:
        ...
        if not selected:
            return (
                ...
                "Either search only the core entity name once, or stop searching and provide the Final Answer from current evidence.\n"
                "Thought: Give the final answer now unless you have a genuinely different entity to search.\n"
            )
    ...
    def _prepare_tool_payload(self, query, source, seq_len_before_commit, base_big_cache, base_small_cache):
        ...
        docs = self._search_wikipedia(query, None, None)
        ...
        if seq_len_before_commit is not None and base_small_cache is not None:
            shadow_state = self._build_shadow_kv(obs_ids, seq_len_before_commit, None, base_small_cache)
        if source == "prefetch":
    ...
    def _consume_prefetch_if_match(self, query):
        ...
        self._invalidate_pending_prefetch("stale", True)
        return None
    ...
    def run_speculative(self, claim, max_gen):
        ...
        few_shot_examples = (
            "Here are examples of FEVER-style fact verification with ReAct. Use Wikipedia search only when evidence is needed.\n\n"
            "--- Example 1: SUPPORTS ---\n"
            "Claim: Berlin is the capital of Germany.\n"
            "Thought: I should verify whether Berlin is identified as the capital of Germany on Wikipedia.\n"
            "Action: SearchWikipedia[Berlin capital of Germany]\n"
            "Observation: Wikipedia search results for 'Berlin capital of Germany':\n"
            "[1] Title: Berlin | Evidence: Berlin is the capital and largest city of Germany...\n"
            "Thought: The evidence directly supports the claim.\n"
            "Final Answer: SUPPORTS\n\n"
            "--- Example 2: REFUTES ---\n"
            "Claim: Stranger Things is set in Bloomington, Indiana.\n"
            "Thought: I should verify the setting of Stranger Things from Wikipedia.\n"
            "Action: SearchWikipedia[Stranger Things setting]\n"
            "Observation: Wikipedia search results for 'Stranger Things setting':\n"
            "[1] Title: Stranger Things | Evidence: Stranger Things is set in the fictional rural town of Hawkins, Indiana, in the 1980s...\n"
            "Thought: The claim says Bloomington, Indiana, but the evidence says Hawkins, Indiana, so the claim is contradicted.\n"
            "Final Answer: REFUTES\n\n"
            "--- Example 3: NOT ENOUGH INFO ---\n"
            "Claim: System of a Down briefly disbanded in limbo.\n"
            "Thought: I should verify whether Wikipedia provides evidence for this specific claim.\n"
            "Action: SearchWikipedia[System of a Down disbanded limbo]\n"
            "Observation: Wikipedia search results for 'System of a Down disbanded limbo':\n"
            "[1] Title: System of a Down | Evidence: The band went on hiatus in 2006 and reunited in 2010...\n"
            "Thought: The retrieved evidence does not support the specific phrasing about briefly disbanding in limbo, so there is insufficient evidence.\n"
            "Final Answer: NOT ENOUGH INFO\n\n"
            "--- Examples End ---\n\n"
        )
        self.system_prompt = (
            "System: You are an elite FEVER fact-verification engine. Solve with ReAct when needed. "
            "Your goal is to classify each claim as SUPPORTS, REFUTES, or NOT ENOUGH INFO using only Wikipedia evidence returned by the tool.\n\n"
            "### TASK DEFINITION ###\n"
            "Given a claim, determine whether the available Wikipedia evidence supports it, contradicts it, or is insufficient.\n"
            "Return exactly one of: SUPPORTS, REFUTES, NOT ENOUGH INFO.\n"
            "### DECISION POLICY (CRITICAL) ###\n"
            "Before every Action, explain in Thought what fact, entity, or relation must be verified.\n"
            "Use the tool when you need factual evidence from Wikipedia. Do not invent evidence.\n"
            "If the evidence directly confirms the claim, answer SUPPORTS.\n"
            "If the evidence directly contradicts the claim, answer REFUTES.\n"
            "If the retrieved evidence is missing, ambiguous, or insufficient to decide, answer NOT ENOUGH INFO.\n"
            "### TOOL RULES ###\n"
            ...
            "Always end with exactly one line in the format: Final Answer: SUPPORTS, REFUTES, or NOT ENOUGH INFO\n"
            f"{few_shot_examples}"
            f"Claim: {claim}\n"
        ...
        def process_token(t_val, phase, action_buf, latest_plan_query, allow_trigger):
                    ...
                    if latest_plan_query:
                        if latest_plan_query == normalized_query:
                            ...
                            if normalized_query != self.accepted_action_prefetch_query:
                                submitted = self._submit_prefetch_candidate(current_action_plan, "action")
                                if submitted:
        ...
        def maybe_prefetch_from_draft(token_id, draft_phase, draft_action_buffer, draft_latest_plan_query, prefetch_allowed, hidden_probe_mode):
            ...
            if hidden_probe_mode and ready_for_probe:
                ...
                if normalized_action_query != self.hidden_probe_query:
                    submitted = self._submit_prefetch_candidate(cleaned_action_query, "hidden_probe")
                    if not submitted:
                        self.prefetch_stats["hidden_probe_prefetch_duplicate"] += 1
                ...
                if draft_latest_plan_query:
                    if draft_latest_plan_query == normalized_action_query:
                        ...
                        if normalized_action_query != self.draft_action_prefetch_query:
                            submitted = self._submit_prefetch_candidate(cleaned_action_query, "action")
                            if submitted:
        ...
        while (input_ids.shape[1] - start_len) < max_gen:
            ...
            if entropy > self.threshold:
                big_logits = self._maybe_boost_action_start(big_logits, current_phase, action_buffer, None, 6.0)
                ...
                small_logits = self._maybe_boost_action_start(small_logits, current_phase, action_buffer, None, 7.5)
        ...
        # Count reasoning steps for ReAct/ReWoo-style analysis.
        ...
        # Count generated LLM tokens without observation payload tokens.
        ...
        self.last_run_stats = {
            ...
            "generation_trace": self._render_trace_summary(1600),
        }
...
def load_fever_dataset(dataset_path: str):
    print(f">>> Loading FEVER dataset: {dataset_path}")
    if dataset_path.endswith(".jsonl"):
        ...
        try:
            ...
            raise RuntimeError(
                f"Cannot read Arrow file directly: {dataset_path}\n"
                f"Please ensure `datasets`/`pyarrow` is installed, or verify that the file is a HuggingFace datasets Arrow shard.\n"
                f"Original error: {e}"
            )
    else:
        try:
...
CLAIM_FIELD_CANDIDATES = ["claim", "question", "text", "sentence", "query", "sentence1", "premise", "hypothesis"]
LABEL_FIELD_CANDIDATES = ["label", "answer", "labels", "gold_label", "target"]
FEVER_LABEL_SET = {"SUPPORTS", "REFUTES", "NOT ENOUGH INFO"}


def normalize_fever_label(label_value: Any) -> str:
    ...
    if label_text_upper in FEVER_LABEL_SET:
        return label_text_upper
    return label_text


def extract_claim_and_label(entry: Dict[str, Any]):
    claim_value = get_first_available(entry, CLAIM_FIELD_CANDIDATES, "")
    label_value = get_first_available(entry, LABEL_FIELD_CANDIDATES, "")

    claim = "" if claim_value is None else str(claim_value).strip()
    ref_label = normalize_fever_label(label_value)
...
def main(config):
    ...
    try:
        ...
        num_test = min(num_test, len(full_dataset))
        ...
        for i, entry in enumerate(test_data):
            ...
            if not claim:
                print("[Skipped] Unable to parse claim text")
                continue

            print(f"Claim: {claim[:180]}...")
            print(f"Reference Label: {ref_label if ref_label else 'N/A'}")
            ...
            try:
                ...
                print("\n[Full Output Trace]\n" + generated_text)
                print(f"\n[Evaluation] Predicted label: {pred_label} | Reference label: {ref_label if ref_label else 'N/A'} | EM: {'✅' if is_hit else '❌'} | F1: {f1:.3f}")
                if engine.last_run_stats:
                    ...
                    print(
                        "[Dual Async Winner Stats] "
                        f"both_available={engine.last_run_stats['dual_async_both_available']} | "
                        f"first_win={engine.last_run_stats['dual_async_first_win']} | "
                        f"second_win={engine.last_run_stats['dual_async_second_win']} | "
                        f"fallback_action={engine.last_run_stats['dual_async_fallback_action']}"
                    )
                    print(
                        "[Plan-Action Alignment] "
                        f"closed_actions={engine.last_run_stats['action_closed']} | "
                        f"without_plan={engine.last_run_stats['action_without_tool_plan']} | "
                        f"match={engine.last_run_stats['action_plan_match']} | "
                        f"mismatch={engine.last_run_stats['action_plan_mismatch']} | "
                        f"match_rate={engine.last_run_stats['action_plan_match_rate']:.2%}"
                    )
        ...
        if results:
            ...
            if 'ste_saved_wait_ms' in df.columns:
                print(
                    f"STE average saved wait: {df['ste_saved_wait_ms'].mean():.2f} ms | "
                    f"STE hit rate by submission: {df['ste_hit'].sum() / max(1, df['ste_submitted'].sum()):.2%} | "
                    f"STE hit rate by opportunity: {df['ste_hit'].sum() / max(1, df['ste_prefetch_opportunities'].sum()):.2%}"
                )
            if 'tool_plan_ready_rate' in df.columns:
                print(
                    f"ToolPlan ready rate: {df['tool_plan_ready_rate'].mean():.2%} | "
                    f"Plan-Action alignment rate: {df['action_plan_match'].sum() / max(1, (df['action_plan_match'] + df['action_plan_mismatch']).sum()):.2%}"
                )
            if 'dual_async_first_win' in df.columns:
                print(
                    f"Dual async first winner hits: {df['dual_async_first_win'].sum()} | "
                    f"Dual async second winner hits: {df['dual_async_second_win'].sum()} | "
                    f"Dual async fallback-to-Action count: {df['dual_async_fallback_action'].sum()}"
                )
            if 'hidden_probe_hit_rate_by_triggered' in df.columns:
                print(
                    ...
                    f"Hidden Probe hit rate after ready: {df['hidden_probe_hit'].sum() / max(1, df['hidden_probe_ready'].sum()):.2%} | "
                    f"Hidden Probe hit rate after seen: {df['hidden_probe_hit'].sum() / max(1, df['hidden_probe_seen'].sum()):.2%}"
                )
            if valid_count > 0:
